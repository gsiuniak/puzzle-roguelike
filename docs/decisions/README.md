...
| 56 | Full-canvas backgrounds + large panel arts are PRE-BAKED at physical resolution (bake-and-blit: `CanvasApp` bar-fill bake, scrim stops cache, smooth `getScaled` with stable-size guards, debounced `clearScaledCache`); `imageSmoothingQuality` must be restored with `imageSmoothingEnabled` (2026-08-27) | [56-full-canvas-and-panel-art-are-pre-baked.md](./56-full-canvas-and-panel-art-are-pre-baked.md) |
| 57 | The board fall animation syncs to the controller's FALL phase (`getFallDurationMs` → `setFallDurationMs`) — view animations mirroring controller phases derive their duration from the controller, never a local copy (2026-08-27) | [57-fall-animation-syncs-to-the-fall-phase.md](./57-fall-animation-syncs-to-the-fall-phase.md) |
| 58 | The title screen is video-driven; the transition movie's still-playing `<video>` is HANDED OFF to CharacterSelectScene (`setEntryVideoOverlay` + instant `switchTo`, ownership transferred BEFORE the switch) for a true movie→UI cross-fade — no black fade, no SceneManager rebuild (2026-08-27) | [58-title-screen-is-video-driven-with-a-video-handoff.md](./58-title-screen-is-video-driven-with-a-video-handoff.md) |
| 59 | Direct damage (skull/skill, tagged via `source` on floatingStatEvents) is DELIVERED by visual carriers — skull streams / spell projectiles — with the whole impact suite (counter tick, recoil, hit flash, directional shake, vignette) firing at ARRIVAL; incidental damage stays immediate; the new one-shot `skillCastEvents` drives cast feedback (2026-08-30) | [59-direct-damage-is-delivered-by-visual-carriers.md](./59-direct-damage-is-delivered-by-visual-carriers.md) |

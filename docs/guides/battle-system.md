...
**Pure logic.** Match analysis (returns MatchAnalysis, does NOT modify board), damage application (armor→block→HP), skill effect type constants, skull damage formulas (matched vs destroyed), shared tile reward computation
...
| [#42](../decisions/42-match-4-resolves-get-an-emphasis-freeze.md) | Match-4+ freeze beat + flourish | `_match4FreezeMs` hit-stop; skill-cast sounds pre-checked and HELD when the cast will flourish. |

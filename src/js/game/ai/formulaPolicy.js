/**
 * game/ai/formulaPolicy.js — the DETERMINISTIC formula policy.
 *
 * PROMOTED from sim/toolbench/formula.mjs (2026-07-08) so the LIVE GAME can
 * use the trained champion player for the in-battle hint system
 * (BattleController.getSuggestedAction). sim/toolbench/formula.mjs is now a
 * re-export shim — the trainer/bench/CLI stack is unchanged. This module is
 * BROWSER-SAFE (no node: imports) and must stay that way: it runs raw in the
 * game, in the bench's web workers, and under node.
 *
 * WHY THIS DESIGN: the engine-preview search policy (sim/toolbench/policy.mjs)
 * evaluates candidates through real previews that include refill randomness.
 * Even with common-random-number seeding, different actions consume different
 * draws, so every comparison carries noise — and over ~150 decisions/run the
 * argmax picks noise often enough to cost ~2.5× deployment strength (measured:
 ...
 * via per-effect formulas, and TARGETED destroy/convert casts via per-target
 * no-refill simulation (each fracture row is a candidate, evaluated exactly,
 * zero randomness).
 *
 ...
 * training surface (sim/toolbench/train.mjs --evaluator formula).
 *
 * DETERMINISTIC CHAIN SEARCH: when a candidate's settle triggers an extra
 * turn, the flat `extraTurn` weight is REPLACED by the discounted value of
 * the best FOLLOW-UP action, evaluated on the settled NO-REFILL board with
 * post-action mana (a strict, exactly-deterministic lower bound — holes sit
 * at the top after gravity, so fewer tiles ⇒ conservative). This is
 ...
 *
 * INPUT CONTRACT (the "battle" seam — see sim/toolbench/engine.mjs POLICY
...
import { MANA_COLORS, isSkull, BOARD_COLS, BOARD_ROWS } from '../TileTypes.js';
...
/** The tracked WORKING champion weights (src/assets/data/ — a GAME asset, so
 *  the Vite build ships it verbatim). Node: `loadChampionWeights()`
 *  (sim/toolbench/weights-node.mjs — readFileSync accepts this file URL).
 *  Browser: this URL resolves over http — fetch it and pass the JSON through
 *  `loadFormulaWeights` (the game does this in game/ai/hintWeights.js). */
export const CHAMPION_WEIGHTS_PATH = new URL('../../../assets/data/formula-champion.json', import.meta.url);
...
/** Positions of a row/column/area on the live board. (Exported for the hint
 *  UI's target-area ghost — BattleScene outlines the cells a suggested
 *  targeted cast would hit.) */
export function targetPositions(board, kind, target, radius = 1) {
  const out = [];
  if (kind === 'row') {

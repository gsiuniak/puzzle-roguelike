...
export default class MapGenerator {
  ...
  static _assignTypes(allNodes, depthNodes, depthCount, rng) {
    for (let d = 1; d < depthCount - 1; d++) {
      ...
      // Determine elite eligibility
      const eliteOk = d >= 4;

      // Determine rest eligibility
      const restOk = d >= 3;
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (eliteOk && roll < 0.10) {
          ...
          // ~15% chance for chest on optional branches
          // Chests tend to appear more on middle lanes (branch positions)
          node.type = NODE_TYPES.CHEST;
        } else if (roll < 0.44) {
          // ~7% chance for training
        ...
        if (count === 1 && node.type === NODE_TYPES.CHEST) {
          node.type = NODE_TYPES.BATTLE;
        }
      }
    }

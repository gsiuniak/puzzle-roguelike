...
| [`src/js/scenes/SkillWeaveScene.js`](src/js/scenes/SkillWeaveScene.js) | `SkillWeaveScene` | **"Weave a Power" skill reward screen** (full-canvas ritual, modeled on TitleScreen/GameOverScene). Replaces the "choose 1 of 3 finished skills" reward with a staged **tag draft**: the player builds a skill by picking one keyword tag per step, filling a fixed-length recipe (`[action, element, shape]` today). The background is **toggleable via the `USE_BACKGROUND_VIDEO` constant** (currently `false` → static `skill_weave_background` image). When `true` it plays a looping **video** (`video_skill_select_screen_bg.mp4` at `assets/audio/video/`, an off-DOM muted `<video>` drawn full-canvas each frame in `renderBackground` — same approach as BossIntroScene/VideoPortrait, NOT an AssetManager entry; `_createBackgroundVideo`/`_destroyBackgroundVideo` on enter/exit), with the static image as the until-decoded / autoplay-blocked fallback. ALL interactive UI (3 triangle-arranged tag option plaques, the Recipe container + 3 slots, and the Back/Confirm buttons) is drawn + hit-tested manually in design space via `_computeLayout()` (single source of truth shared by `render()` and input). **Interaction:** clicking a tag option **immediately commits** it into the next recipe slot and advances to the next step (hovering an option grows it slightly + adds a glow as the "this is what a click takes" cue); **Back** (disabled on step 0) removes the most recent tag and restores the previous step's exact options (kept on the `_steps` stack); **Confirm** is disabled until the recipe is full, then becomes "Weave Power" → `_finishWeave()` (it is ONLY the final resolve action, not a per-tag commit). The valid tag pool per step comes from [`skillWeaveTags.js`](src/js/data/skillWeaveTags.js) (`getValidTagsForStep` → `sampleTags` shows up to 3), so every path resolves to a real combo — no dead ends. **Skills aren't implemented yet, so `_finishWeave` is a placeholder** that logs the woven recipe, fires the optional `_onComplete` callback, and returns to `_returnScene`. Configured via `configure({ onComplete, returnScene })`; MapScene routes the `training` node here (see decision #31) and wires `onComplete` → `_handleBattleComplete` so the node completes on return. `onEnter` cross-fades music to `skill_weave_theme` (via `AudioManager.playMusic`); tag-pick plays `sfx_choose_tag`, Back plays `sfx_choose_tag_back`. Registered once in [`main.js`](src/js/main.js). |
...
|------|---------|---------|
| [`src/js/data/skillWeaveTags.js`](src/js/data/skillWeaveTags.js) | `SKILL_WEAVE_TAGS`, `RECIPE_LENGTH`, `TAG_CATEGORY`, `DEFAULT_TAG_ICON`, `getTag`, `getTagLabel`, `getTagIcon`, `getValidTagsForStep`, `isRecipeComplete`, `sampleTags` | **Tag catalog + recipe grammar for the Skill Weave screen** ([`SkillWeaveScene`](src/js/scenes/SkillWeaveScene.js)). `SKILL_WEAVE_TAGS` is an id-keyed catalog (`{ id, label, category, icon }`) of element tags (red→Fire, blue→Water, green→Leaf, yellow→Sun, purple→Moon, skull), action tags (damage/armor/create/destroy/convert/gain/heal), and shape/target tags (row/column/area/tile/random). The **grammar** is a small per-action rule table (`ACTION_RULES`, internal): each action declares its allowed `elements` + `shapes`, so a complete recipe is `[action, element, shape]` and `getValidTagsForStep(chosen)` derives the valid next-step pool (step 0 → actions, 1 → that action's elements, 2 → its shapes) — every selectable path leads to a real combo, no dead ends. `sampleTags(pool, n)` (Fisher–Yates) picks the ≤3 shown per step. Icons fall back to `DEFAULT_TAG_ICON` (`skill_weave_tag_test`, the gold flame placeholder) until per-tag art exists. **To extend:** add/adjust an action's elements/shapes or add a tag — no scene changes needed. |
| [`src/js/data/keywordDefinitions.js`](src/js/data/keywordDefinitions.js) | `KEYWORD_DEFINITIONS` (default), `KEYWORD_ALIASES`, `KEYWORD_COLOR`, `KEYWORD_MISSING_COLOR`, `getKeywordDefinition`, `normalizeKeywordKey` | **Centralized inline-keyword catalog.** Each entry `{ id, label, description }`, keyed by normalized id (lowercase, single-spaced). **Styling is NOT per-keyword** — ALL keywords render in the single shared `KEYWORD_COLOR` (change it in one place to retint every keyword); missing keywords use `KEYWORD_MISSING_COLOR`. Any UI description (relic/skill/enemy/stat/tutorial text) may mark a word with `[[Label]]` markup → resolved case-insensitively (with `KEYWORD_ALIASES` for alt spellings) so it renders bracket-free, colored, and gets an auto tooltip. Descriptions may themselves contain `[[keywords]]` (powers chained tooltips). Seeded: create, tiles, match, skulls, mana, damage, armor, heal, attack, extra turn, thrall, harvest. Parsed by [`keywordParser.js`](src/js/systems/keywordParser.js), rendered by [`KeywordText`](src/js/ui/KeywordText.js). |
...
| "Weave a Power / skill tag screen" | [`SkillWeaveScene`](src/js/scenes/SkillWeaveScene.js) | Tag pool/grammar in [`skillWeaveTags.js`](src/js/data/skillWeaveTags.js); entry via [`MapScene._transitionToSkillWeave`](src/js/scenes/MapScene.js) (training node) |
| "Add/edit a Weave tag or change valid combos" | [`skillWeaveTags.js`](src/js/data/skillWeaveTags.js) (catalog + `ACTION_RULES` grammar) | Scene reads it generically — no [`SkillWeaveScene`](src/js/scenes/SkillWeaveScene.js) change needed |
...
29. **Descriptions support data-driven inline keywords with chained tooltips.** Any UI description string may embed `[[Keyword]]` markup. The pipeline is fully reusable and lives outside any individual renderer:
    ...
    - **Inline keyword tooltips ("anywhere")** — any scene with a `TooltipManager` can call `attachKeywordSource(keywordTextEl, opts)` to make a description's individual keyword spans hoverable. Wired today in **BattleScene** (`_registerSkillKeywordSources` — player + enemy skill descriptions) and **CharacterSelectScene** (`_registerKeywordTooltips` — skill/relic/character descriptions; the scene owns its own `TooltipManager`, re-registering on every selection change). Hovering a keyword shows its definition tooltip + that keyword's own chain.
...
31. **The "Weave a Power" skill reward is a staged tag draft, and grammar lives in data — the screen has no skill knowledge.** [`SkillWeaveScene`](src/js/scenes/SkillWeaveScene.js) is a full-canvas scene that replaces the relic-style "pick 1 of 3 finished options" reward for **skills** with a multi-step keyword draft. It is fully decoupled:
    - **Grammar/data** ([`skillWeaveTags.js`](src/js/data/skillWeaveTags.js)) owns the tag catalog AND the valid-pool-per-step logic (`getValidTagsForStep` reading an internal per-action rule table). The scene never hardcodes which tags are valid — it just renders `sampleTags(getValidTagsForStep(recipe), 3)`. A complete recipe is `[action, element, shape]`; extend by editing the rule table, not the scene.
    ...
    - **Entry/exit mirrors the battle contract.** MapScene routes the `training` node via `_transitionToSkillWeave(node)` (serializes traversal into `_savedTraversalState`, `configure({onComplete, returnScene})`, `fadeToScene`). `onComplete` calls the SAME `_handleBattleComplete({result, nodeId})` battle uses, so on return `MapScene.onEnter` completes the node + reveals the next depth identically. (Training previously routed to a debug Goblin battle.)
    - **Placeholder:** skills aren't implemented, so `_finishWeave()` only logs the woven recipe, fires `onComplete`, and returns — the recipe→concrete-skill resolution is the future hook. All layout/visual tunables are named constants atop the scene; the 7 skill_weave assets are registered in [`main.js` ASSET_MAP](src/js/main.js:35) and the tag icon falls back to the gold-flame placeholder until per-tag art exists.
...
| `SKILL_EFFECT_TYPES` | [`MatchResolver.js:23`](src/js/game/MatchResolver.js:23) | damage, armor, destroy_tiles, destroy_tiles_row, create_tiles, convert_tile, convert_tiles_by_type, heal, extra_turn, gain_attack (permanent caster attack +amount, e.g. Encroach), self_destruct (caster hp→0, ends battle on next `_checkGameOver`), drain_mana (remove `amount` of one/every mana color from the OPPONENT — Soul Burn), silence (block opponent skill casts for N of their turns — defined + supported but currently unused by any skill), set_attack (force opponent attack to `value` for N of their turns then restore — Exsanguinate) |
| `TILE_TYPES` | [`TileTypes.js:11`](src/js/game/TileTypes.js:11) | RED, BLUE, GREEN, YELLOW, PURPLE, SKULL, DISEASE, THRALL — each with id, isSkull, color, particleColor, spawnWeight. **DISEASE** is `isInert: true` + `spawnWeight: 0`: it never spawns naturally (placed only by effects, e.g. Infected Tooth), is neither mana nor skull, and matching/destroying it awards no mana/skull damage (MatchResolver skips inert tiles for those), though a 4+ inert match still grants an extra turn. **THRALL** is `isWild: true` + `spawnWeight: 0`: placed only by effects (Usurper's Heart), it never spawns naturally and acts as a **match-anything joker** — in `BoardModel._scanLineRuns` a Thrall substitutes for any concrete color/skull type it lines up beside (Red+Thrall+Red = Red match), but NOT for inert Disease tiles; a wild match resolves to the concrete type (awards that color's mana / skull damage), while a Thrall destroyed WITHOUT a host (raw destroy) awards nothing. Helpers: `isInert(typeId)`, `isWild(typeId)`, and `isMana(typeId)` excludes both inert and wild. |

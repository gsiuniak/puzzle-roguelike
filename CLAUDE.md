...
| [`src/js/ui/RelicBar.js`](src/js/ui/RelicBar.js) | `RelicBar` | Thin passive horizontal strip at the very top of the battle screen. Renders the player's relics as small icons (Slay-the-Spire style), left-aligned with no background or border — the icons float directly over the battle background. Non-interactive (hover/click tooltips reserved for later). Width is computed from the side+center column extent so the bar sits flush against the player panel's left edge and the enemy panel's right edge. Mounted in BattleScene with `margin.bottom = -(height+top)` so the bar visually overlays the top of MainRow without consuming vertical flex space (BattleBoardPanel renders as a `min(w,h)` square, so any vertical shrink would push the side panels horizontally away from the board). `setRelics()` is idempotent — safe to call every frame. |

...
30. **Thrall wild tiles + Baron's Signet harvest are fully data-driven (the Act 1 boss engine).** A wild tile type (`THRALL`, `isWild`) and two enemy relics replace Lord Malakor's old mana-flood engine. The mechanic reuses existing extension points — no one-off boss checks:
    ...
    - **Create (Usurper's Heart)** = `create_tiles` board passive with `avoidMatches:true`. `BattleController._applyPassiveCreateTiles` places tiles one-at-a-time, each preferring a spot where `BoardModel.positionCreatesMatch` is false (re-checked per placement, graceful fallback to any replaceable tile), so fresh Thralls don't auto-resolve into free matches.
    - **Harvest (Baron's Signet)** = `harvest_tiles` board passive. `BattleController._applyPassiveHarvest` counts all Thralls, grants the owner `attackPer` attack each (permanent, same semantics as `gain_attack`), converts them to Skulls in place (no cascade — the Skulls sit as a threat), and queues a one-shot `harvestEvents` entry (Thrall positions captured BEFORE conversion + tendril color).
    - **Pre-action animation:** the harvest fires at the boss's `onTurnStart` (before it acts). `_applyPassiveHarvest` adds `HARVEST_ANIM_DELAY` to `_extraEnemyTurnDelay` (reset each turn intro, consumed when the enemy fires), so the boss waits while `BattleScene._spawnHarvestTendrils` plays [`HarvestTendrilEffect`](src/js/ui/HarvestTendrilEffect.js) — red tendrils spiraling from each harvested Thrall to the enemy portrait. (The board mutation itself is synchronous — the data applies at turn start while the tendrils play; a fully animation-gated apply would need async turn machinery, deliberately avoided.)
    - **Ordering:** Malakor's `relics: ['barons_signet', 'heart_of_usurper']` — `onTurnStart` effects resolve in relic-array order, so each turn he harvests last turn's leftover Thralls, then seeds 3 new ones. Tunables: relic payloads (`createTiles.amount`, `harvestTiles.attackPer`/`toType`/`tendrilColor`), `HARVEST_ANIM_DELAY`, and `HarvestTendrilEffect` ctor config.

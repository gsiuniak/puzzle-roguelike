...
| [`src/js/ui/BoardPlaceholder.js`](src/js/ui/BoardPlaceholder.js) | `BoardPlaceholder` | Board grid rendering: tile sprites, highlights, empty cells, fall animation, swap animation, targeting overlay, particle effects. **Wild (Thrall) tiles** get an animated rainbow-border overlay drawn on top of the tile art via `_drawWildBorder` (data-driven by `isWild(tileId)`, so it appears/disappears automatically as tiles become/stop being wild; follows fall + swap animation). Fully tunable via the `WILD_BORDER_CONFIG` constant (thickness, mode `'cycle'`/`'scroll'`, speed, glow, saturation/lightness, opacity) + `WILD_BORDER_HUES` (the 5 mana colors as HSL hues). |
...
30. **Thrall wild tiles + Baron's Signet harvest are fully data-driven (the Act 1 boss engine).** A wild tile type (`THRALL`, `isWild`) and two enemy relics replace Lord Malakor's old mana-flood engine. The mechanic reuses existing extension points — no one-off boss checks:
    ...
    - **Wild visual indicator** = an animated rainbow border drawn on top of the Thrall tile art by `BoardPlaceholder._drawWildBorder` (the base sprite is untouched). It's purely render-time and data-driven by `isWild(tileId)`, so it appears/vanishes automatically with the tile's wild state and follows fall/swap animation; layered under the highlight/hover/selection overlays. Tunables live in `WILD_BORDER_CONFIG` / `WILD_BORDER_HUES` at the top of [`BoardPlaceholder.js`](src/js/ui/BoardPlaceholder.js).

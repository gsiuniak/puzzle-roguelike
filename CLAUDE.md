...
| [`src/js/ui/BoardPlaceholder.js`](src/js/ui/BoardPlaceholder.js) | `BoardPlaceholder` | Board grid rendering: tile sprites, highlights, empty cells, fall animation, swap animation, targeting overlay, particle effects. **Wild (Thrall) tiles** get the `wild_tile_border` frame image overlaid on top of the tile art via `_drawWildTileBorder` — centered and scaled slightly LARGER than the cell (`WILD_TILE_BORDER_SCALE`) so the frame encompasses the tile (data-driven by `isWild(tileId)`, so it appears/disappears automatically; follows fall + swap animation). Because the frame overhangs the cell, wild borders are drawn in a deferred pass AFTER all tiles so neighbors don't clip them. The legacy animated rainbow-border (`_drawWildBorder` + `WILD_BORDER_CONFIG`/`WILD_BORDER_HUES`) is **disabled** (`enabled:false`) but retained for reference; it's the fallback if the `wild_tile_border` asset is missing. |

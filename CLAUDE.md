...
| [`src/js/ui/HarvestTendrilEffect.js`](src/js/ui/HarvestTendrilEffect.js) | `HarvestTendrilEffect` | **Thrall-harvest tendril animation.** Spirals of energy travel from a set of source board cells to a single target point (a portrait), then flare + fade. Spawned by `BattleScene._spawnHarvestTendrils` from `getState().harvestEvents` (Baron's Signet): red tendrils spiral from each harvested Thrall tile to the boss portrait. Same externally-managed effect contract as `FloatingTextEffect` (`update(dt)`/`render(ctx)`/`done`); lives in `_floatingEffects`. All feel tunables (color, spiralTurns, amplitude, durations) are ctor config. |

...
| [`src/js/ui/VideoPortrait.js`](src/js/ui/VideoPortrait.js) | `VideoPortrait` | **Live white-keyed video portrait.** Subclass of `UIImage` whose "image" is the current frame of a muted, looping, off-DOM `<video>` with all near-white pixels chroma-keyed to transparent. Each render it draws the frame into an offscreen canvas (capped to `PROCESS_MAX_DIM`, `willReadFrequently`), zeroes the alpha of every pixel whose RGB are all ≥ `whiteThreshold` (default `WHITE_THRESHOLD` 245 — tolerant of mp4 compression near #FFF), and returns that canvas from `getImage()` so `UIImage.renderSelf` cover-fits it like a normal sprite (keyed white → panel shows through). Falls back to the static portrait sprite (ctor `fallbackAssetKey`) until the first frame decodes. Constructed by `CharacterInfoPane` when its data carries `portraitVideo`; `destroy()` releases the `<video>` (called on pane rebuild and `BattleScene.onExit`). Not an AssetManager entry — owns its own `<video>` (like `BossIntroScene`). |

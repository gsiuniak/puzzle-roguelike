...
29. **Descriptions support data-driven inline keywords with chained tooltips.** Any UI description string may embed `[[Keyword]]` markup. The pipeline is fully reusable and lives outside any individual renderer:
    - **Definitions** ([`keywordDefinitions.js`](src/js/data/keywordDefinitions.js)) — centralized `{ id, label, color, description }` catalog keyed by normalized id (+ `KEYWORD_ALIASES`). This is the single place to add/tune keywords and their styling.
    ...
    - **Rendering** ([`KeywordText`](src/js/ui/KeywordText.js)) — a `UIText`-compatible element that draws colored, keyword-aware wrapped text and is now the body of `Tooltip` and the description of `RewardOptionPanel`. So coloring is automatic anywhere those are used; no per-renderer keyword code.
    - **Chained tooltips** ([`TooltipManager`](src/js/systems/TooltipManager.js)) — hovering/touch-holding a relic shows its parent tooltip AND auto-spawns a chain of child keyword tooltips (one per keyword found in the parent's text, then keywords nested inside those, BFS). Each child is positioned relative to the PREVIOUS tooltip panel (not the relic), so the chain grows toward screen center, clamped to the viewport. Recursion is bounded by `MAX_TOOLTIP_CHAIN_DEPTH` + a visited-id set; spacing is `KEYWORD_TOOLTIP_CHAIN_GAP`. Hiding the parent hides the whole chain. **Existing relic tooltip behavior is unchanged for keyword-free descriptions** — chaining is purely additive. Enabled by default; opt out per-attachment with `options.keywordChain === false`.

...
31. **The "Weave a Power" skill reward is a staged tag draft, and grammar lives in data — the screen has no skill knowledge.** [`SkillWeaveScene`](src/js/scenes/SkillWeaveScene.js) is a full-canvas scene that replaces the relic-style "pick 1 of 3 finished options" reward for **skills** with a multi-step keyword draft. It is fully decoupled:
    ...
    - **Draft state** is a `_recipe` (committed tag ids) + a `_steps` stack of `{options, selected}` per step. Committing pushes onto `_recipe` and builds the next step; **Back** pops `_recipe` and truncates `_steps` to `_recipe.length + 1`, which restores the previous step's exact options + selection (they were never discarded). Selection (click) is separate from commit (Confirm) so Back/Confirm have clean semantics; clicking a tag only highlights it.

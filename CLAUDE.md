...
| [`src/js/ui/Tooltip.js`](src/js/ui/Tooltip.js) | `Tooltip` | **Reusable floating tooltip visual.** Background asset `tooltip_panel` + optional bold `title` at the top + centered, word-wrapped body `UIText`. When `title` is set it sits top-aligned inside the padded inner area and the body fills the remaining space below (separated by `titleGap`); when empty, the body fills the whole inner area centered (legacy behavior). Aspect ratio is preserved — only width (or `scale` multiplier) is configurable; height is derived as width / native aspect so the art never stretches. Driven externally by `TooltipManager` (not added as a child of any container). `setOptions({text, title, scale, width, padding, fontSize, lineHeight, color, titleFontSize, titleColor, titleBold, titleGap})` + `setPosition(x, y)` + `render(ctx)`. |

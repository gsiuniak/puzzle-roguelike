...
| [`src/js/ui/FloatingTextEffect.js`](src/js/ui/FloatingTextEffect.js) | `FloatingTextEffect` | Floating text (grow/settle/hold/fade) at a screen coord. Used for "+N" match-count text AND combat-stat text over portraits: red "-x" damage / green "+x" heal / blue "+x" armor (spawned by `BattleScene._spawnStatTextEffect` from `getState().floatingStatEvents`). |

...
| `TILE_TYPES` | [`TileTypes.js:11`](src/js/game/TileTypes.js:11) | RED, BLUE, GREEN, YELLOW, PURPLE, SKULL, DISEASE — each with id, isSkull, color, particleColor, spawnWeight. **DISEASE** is `isInert: true` + `spawnWeight: 0`: it never spawns naturally (placed only by effects, e.g. Infected Tooth), is neither mana nor skull, and matching/destroying it awards no mana/skull damage (MatchResolver skips inert tiles for those), though a 4+ inert match still grants an extra turn like any other big match. Helpers: `isInert(typeId)`, and `isMana(typeId)` now excludes inert. |
...
| One-shot visual/SFX flags | [`BattleController.getState()`](src/js/game/BattleController.js:207) | Read-and-clear pattern: extraTurnTriggerPos, shakeIntensity, skullDamageDealt, pendingSkillSound, destroyedTiles, convertedTiles, matchTextTriggers, floatingStatEvents, relicTriggers |

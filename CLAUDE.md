...
| [`src/js/ui/BattleScene.js`](src/js/ui/BattleScene.js) | `BattleScene` | Battle layout: a thin `RelicBar` strip at the very top (player relics, passive) over a 3-column main row (compact stacked player column \| board+combat-log center \| compact stacked enemy column). Input handling, visual effects, game-over → reward overlay transition. Hidden turn label retained for backwards-compat (state/data binding only — visual display disabled). |
...
| [`src/js/ui/RelicBar.js`](src/js/ui/RelicBar.js) | `RelicBar` | Thin passive horizontal strip at the very top of the battle screen. Renders the player's relics as small icons (Slay-the-Spire style), left-aligned with no background or border — the icons float directly over the battle background. Non-interactive (hover/click tooltips reserved for later). Width is computed from `SIDE_COL_MIN_WIDTH + MAIN_ROW_GAP + CENTER_COL_WIDTH + MAIN_ROW_GAP + SIDE_COL_MIN_WIDTH` so the bar sits flush against the player panel's left edge and the enemy panel's right edge. The bar consumes vertical layout space normally, and BattleScene shrinks `CENTER_COL_WIDTH` by `RELIC_BAR_VERTICAL_FOOTPRINT` to keep the BattleBoardPanel's `min(w,h)` square correctly proportioned (otherwise the board would shrink vertically and expose a gap between the side panels and the board). `setRelics()` is idempotent — safe to call every frame. |

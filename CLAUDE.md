...
src/
  ...
  js/
    ...
    data/                 — gameplay data definitions
      ...
      enemies/            — enemy defs organized by act (act1/ act2/ act3/, each
                            with its own index) + top-level index w/ spawn selection
...
| [`src/js/data/enemies/act1/`](src/js/data/enemies/act1/) · [`act2/`](src/js/data/enemies/act2/) · [`act3/`](src/js/data/enemies/act3/) | each: per-enemy files + `index.js` (array of that act's defs) | **Per-act enemy rosters.** Each enemy file `export default`s a def carrying categorization fields **`act` (1\|2\|3)**, **`rarity` (common\|uncommon\|rare)**, **`type` (minion\|elite\|boss)** plus `hp`/`maxHp`/`attack`/`armor`, skills by ID, **relics by ID from the ENEMY-ONLY pool**, optional `aiBehavior` + `music`. Act 1: goblin/goblinBrute/goblinWarlord. Act 2: orc/ogre. Act 3: wraith/lich. (New enemies reuse `portrait_goblin` as placeholder art.) |
| [`src/js/data/enemies/index.js`](src/js/data/enemies/index.js) | `getEnemyById`, `getEnemiesByAct`, `getEnemiesByType`, `getEnemiesByRarity`, `getActForDepth`, `enemyTypeForNodeType`, `selectEnemyForNode`, `ENEMIES_BY_ACT`, `ALL_ENEMIES`, `goblin`, default: `ENEMIES_BY_ID` | **Enemy registry + spawn selection.** Aggregates the per-act rosters. `selectEnemyForNode({depth, nodeType})` is the spawn entrypoint MapScene uses: maps depth→act (10 depths/3 acts → act1=0–3, act2=4–6, act3=7–9 via `getActForDepth`) and node type→enemy type (battle→minion, elite→elite, boss→boss via `enemyTypeForNodeType`), then rarity-weighted-randomly picks a matching enemy (falls back to an act minion, then goblin). Returns a shared catalog ref — callers deep-clone. |
...
| "Add new enemy" | new file in the matching [`data/enemies/actN/`](src/js/data/enemies/) folder (with `act`/`rarity`/`type` fields) + add to that act's `index.js` array | Spawning is automatic via [`selectEnemyForNode`](src/js/data/enemies/index.js) (act/type matched). Register portrait/skill assets in [`main.js` ASSET_MAP](src/js/main.js:35) |
...
| "Change which enemy spawns where" | [`selectEnemyForNode` / `getActForDepth` / `enemyTypeForNodeType`](src/js/data/enemies/index.js) | Depth→act + node-type→enemy-type mapping + rarity weights all live here |
